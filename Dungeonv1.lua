...
local speedList = {"Normal", "Fast"}
local speedMap = {
    Normal = 0.175,
    Fast = 0.1
}

SpeedDropdown = Tab:Dropdown({
    Title = "Auto Dungeon Speed",
    List = speedList,
    Value = ConfigState.AutoDungeonSpeed or "Normal",
    Callback = function(choice)
        ConfigState.AutoDungeonSpeed = choice
        getgenv()._autoDungeonSpeedValue = speedMap[choice] or 0.175
    end
})

getgenv()._autoDungeonSpeedValue = speedMap[ConfigState.AutoDungeonSpeed or "Normal"] or 0.175

Toggle = Tab:Toggle({
    Title = "Enable Auto Dungeon",
    Desc = "Automatically farm dungeons with pet",
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            -- ตั้งค่าความเร็ว thread ให้ตรง dropdown ทันที
            getgenv()._autoDungeonSpeedValue = speedMap[ConfigState.AutoDungeonSpeed or "Normal"] or 0.175
            ...
            if not getgenv()._dungeonFarmThread or not getgenv()._dungeonFarmThreadAlive then
                ...
                getgenv()._dungeonFarmThread = coroutine.create(function()
                    ...
                    while getgenv()._dungeonFarmThreadAlive do
                        -- ดึง speed เฉพาะในขณะรัน เพื่อรองรับ dropdown เปลี่ยนกลางทาง
                        local waitSpeed = getgenv()._autoDungeonSpeedValue or 0.175
            ...
            task.wait(getgenv()._autoDungeonSpeedValue or 0.175)
            local HRP = getHRP()
            if HRP then
                local bodyVelocity = HRP:FindFirstChild("AntiGravity")
                if bodyVelocity then
